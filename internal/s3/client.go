...
func (c *Client) Upload(ctx context.Context, data []byte, filename string) (string, error) {
	...
	canonicalRequest, stringToSign := c.buildSignatureData(req, payloadHash, now)
...
func (c *Client) buildSignatureData(req *http.Request, payloadHash string, now time.Time) (string, string) {
